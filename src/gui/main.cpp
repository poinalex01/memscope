...
int main(int argc, char* argv[]) {
  ...
  QPushButton* attachButton =
      new QPushButton("Attach to Notepad", centralWidget);
  QPushButton* scanButton =
      new QPushButton("Scan Own Process for 123456789", centralWidget);
  QPushButton* rescanButton =
      new QPushButton("Change Value and Rescan", centralWidget);
  ...
  QObject::connect(scanButton, &QPushButton::clicked, [&, outputBox]() {
    ...
    for (const auto& region : regions) {
      auto found =
          memscope::ScanRegionForValue(processHandle, region, currentValue);
  ...
  QObject::connect(rescanButton, &QPushButton::clicked, [&, outputBox]() {
    ...
    outputBox->append(
        QString("Rescan narrowed to %1 matches.").arg(matches.size()));
  });

  QObject::connect(
      freezeButton, &QPushButton::clicked, [&, outputBox, freezeButton]() {
        if (matches.empty()) {
